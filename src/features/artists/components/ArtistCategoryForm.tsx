...
export function ArtistCategoryForm({ categoryId }: ArtistCategoryFormProps) {
  const router = useRouter();
  const isEditing = !!categoryId;
  const [isLoading, setIsLoading] = useState(false);
  
  const [formData, setFormData] = useState<Partial<ArtistCategory>>({
    name: '',
    slug: '',
    description: '',
    icon: '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let result;
    if (isEditing) {
      result = await artistApi.updateCategory(categoryId!, formData);
    } else {
      result = await artistApi.createCategory(formData);
    }

    setIsLoading(false);
    
    if (result) {
      router.push('/admin/content/artists/categories');
    } else {
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <textarea 
              ...
              onChange={handleChange}
              className="w-full px-4 py-2 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="Brief description of this artist category"
            />
          </div>
          
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">Icon (SVG Code)</label>
            <textarea 
              name="icon" 
              rows={2}
              value={formData.icon || ''} 
