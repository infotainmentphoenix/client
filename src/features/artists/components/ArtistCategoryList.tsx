  ...
  return (
    <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
      <div className="p-6 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold tracking-tight">Artist Categories</h2>
          <p className="text-sm text-gray-500">Manage genres and types of artists.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/admin/content/artists/categories/new" className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap">
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={4} className="px-6 py-12 text-center">
                  ...
                  <p className="text-gray-500 text-sm">Add categories like Singers, Bands, DJs, etc.</p>
                </td>
              </tr>
            ) : (
                ...
                <tr key={cat.id} className="hover:bg-foreground/[0.02] transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-semibold text-foreground flex items-center gap-2">
                      {cat.icon && (
                        <div dangerouslySetInnerHTML={{ __html: cat.icon }} className="w-5 h-5 text-gray-500" />
                      )}
                      {cat.name}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link href={`/admin/content/artists/categories/${cat.id}`} className="p-2 text-gray-400 hover:text-blue-600 transition-colors rounded-lg hover:bg-blue-50 dark:hover:bg-blue-500/10">
