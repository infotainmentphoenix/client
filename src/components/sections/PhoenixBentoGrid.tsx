      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
        ...
        <ScrollReveal delay={0}>
          <GlowCard href="/about" className="h-[500px] bg-gray-900 flex flex-col justify-end p-8 md:p-10 w-full">
            <div 
              className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-40 group-hover:opacity-70 group-hover:scale-110 transition-all duration-700 ease-in-out"
            ...
            <div className="absolute top-8 right-8 text-black bg-white w-12 h-12 rounded-full flex items-center justify-center scale-90 opacity-80 group-hover:scale-110 group-hover:opacity-100 transition-all duration-300 shadow-lg group-hover:bg-blue-500 group-hover:text-white">
          ...
          <GlowCard className="h-[500px] bg-white dark:bg-[#0c0c0c] p-8 md:p-10 flex flex-col w-full">
            ...
            <div className="flex flex-wrap gap-3 mt-auto relative z-10 overflow-y-auto flex-1 content-start pb-4 hide-scroll" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              ...
              {['Corporate Events', 'Weddings', 'Concerts', 'Photography', 'Videography', 'Live Streaming', 'Artist Management', 'Stage Setup'].map((tag, i) => (
                <Link href="/services" key={tag} className="px-4 py-2.5 rounded-full border border-black/10 dark:border-white/10 text-sm font-medium text-gray-700 dark:text-gray-300 bg-black/5 dark:bg-white/5 hover:bg-black dark:hover:bg-white hover:text-white dark:hover:text-black hover:border-black dark:hover:border-white cursor-pointer transition-all duration-300" style={{ animationDelay: `${i * 100}ms` }}>
                  {tag}
                </Link>
              ))}
            </div>
          ...
          <GlowCard className="h-[500px] bg-white dark:bg-[#0c0c0c] p-8 md:p-10 flex flex-col w-full">
            ...
            <div className="flex flex-col gap-3 relative z-10 overflow-y-auto flex-1 pb-4 hide-scroll" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              ...
              {[
                ...
                <Link href="/artists" key={idx} className="flex flex-col bg-gray-50 dark:bg-white/5 rounded-2xl px-6 py-4 hover:bg-black dark:hover:bg-white/10 hover:text-white transition-colors duration-300 cursor-pointer group/item border border-black/5 hover:border-black/10 dark:hover:border-white/10 shrink-0">
