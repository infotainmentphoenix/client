...
export default function GoogleAuthSimulationPage() {
  const router = useRouter();
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    // Simulate OAuth redirect handling
    const handleGoogleLogin = async () => {
      try {
        await authApi.googleLogin('simulated_oauth_token_from_url');
        ...
        {error ? (
          ...
          <div className="flex flex-col items-center">
            <div className="relative mb-8">
              <GoogleIcon className="w-16 h-16 relative z-10" />
              <div className="absolute inset-0 bg-blue-500/20 blur-xl rounded-full animate-pulse" />
            </div>
            <div className="flex items-center gap-3 text-gray-700 dark:text-gray-300">
              <LoaderIcon className="w-5 h-5 animate-spin text-blue-500" />
              <span className="font-medium text-lg">Authenticating with Google...</span>
            </div>
